...
################################################################################
tryCatch({sql <- paste0("select attribute_name, ROUND(count(attribute_name)*100/convert(float,(select count(*) from @cdm_database_schema.death)),1) as ratio 
...
################################################################################
tryCatch({sql <- paste0("SELECT (SELECT CONCEPT_NAME FROM @cdm_database_schema.concept where concept_id = death_type_concept_id) as attribute_name,
