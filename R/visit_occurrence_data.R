...
#sd Graph????
################################################################################
tryCatch({sql <- "select	LEFT(visit_end_date,4) as visit_year, count(person_id) as person_count FROM @cdm_database_schema.visit_occurrence
GROUP BY LEFT(visit_end_date,4)"
